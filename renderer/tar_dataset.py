...
class TarShardDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Find a valid positive sample. Some clips may have empty/malformed landmark files.
        valid = False
        for offset in range(len(self.meta_list)):
            real_idx = (idx + offset) % len(self.meta_list)
            meta = self.meta_list[real_idx]
            frames = meta["frames"]
        ...
        if not valid:
            raise RuntimeError("No valid sample found in TarShardDataset.")
        ...
        # Find a valid negative sample. If failed after several attempts, fall back to image_0.
        neg_image = image_0
